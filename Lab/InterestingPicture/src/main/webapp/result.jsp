...
<html>
    ...
    <body>
        <% if (request.getAttribute("pictureURL") != null) { %>
            <% if (request.getParameter("searchWord").toLowerCase().equals("andy")) { %>
            <h1>Here is an interesting picture of a <%= request.getParameter("searchWord")%></h1><br>
                <% for (int i = 0; i < 10 ; i++) { %>
                <img src = "https://upload.wikimedia.org/wikipedia/commons/thumb/0/09/Andrew_Carnegie%2C_by_Theodore_Marceau.jpg/1920px-Andrew_Carnegie%2C_by_Theodore_Marceau.jpg" width="100" height="100"><br><br>
                <% } %>
            <% }  else { %>
            <h1>Here is an interesting picture of a <%= request.getParameter("searchWord")%></h1><br>
